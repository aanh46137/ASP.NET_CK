...
using LuyenTapCK2.Models;

namespace LuyenTapCK2.Controllers
{
    public class TiepViensController : Controller
    {
        ...
        [HttpPost]
        public ActionResult Login(string tiepvien, string matkhau)
        {
            var user = db.TiepVien.Where(u => u.HoTen == tiepvien && u.MatKhau == matkhau).FirstOrDefault();
            if (user == null)
            {
